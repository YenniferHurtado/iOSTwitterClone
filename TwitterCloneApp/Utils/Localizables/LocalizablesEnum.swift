//
//  Localizables.swift
//  TwitterCloneApp
//
//  Created by Yennifer Hurtado Arce on 19/08/22.
//

enum Localizable {
    
    //MARK: LOGIN VIEW
    static let main_text_login_label = "main_text_login_label"
    static let continue_google = "continue_google";
    static let continue_apple = "continue_apple";
    static let create_account = "create_account";
    static let terms_policy_cookies_label = "terms_policy_cookies_label";
    static let terms_label = "terms_label";
    static let privacy_policy_label = "privacy_policy_label";
    static let cookies_policy_label = "cookies_policy_label";
    static let already_have_account_button = "already_have_account_button";
    static let login_button = "sign_in_button";
        // - Terms Links
    static let terms_link = "terms_link";
    static let privacy_link = "privacy_link";
    static let cookies_link = "cookies_link";
    
    //MARK: REGISTER VIEW
    static let cancel_button = "cancel_button"
    static let main_text_register_label = "main_text_register_label"
    static let username_placeholder = "username_placeholder"
    static let forget_password_button = "forget_password_button"
    static let next_button = "next_button"
    static let log_in_button = "log_in_button"
    
    //MARK: EXPLORE VIEW
    static let explore_nav_title = "explore_nav_title"
    
    //MARK: Notification View
    static let notification_nav_title = "notification_nav_title"
    //MARK: Messages View
    static let messages_nav_title = "messages_nav_title"
}
